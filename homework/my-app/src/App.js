...
/*
const [state, stateTurn] = useState(1);
const buttons = () => {
    switch (state) {
      case 1:
        return <button>кнопочка 1</button>;
      case 2:
        return <button>кнопочка 2</button>;
      default:
        return <button>кнопочка 3</button>;
    }
  };

  <button onClick={()=> stateTurn(prevState => (prevState === 1 ? 2 : 1)) }>Кнопка</button>
